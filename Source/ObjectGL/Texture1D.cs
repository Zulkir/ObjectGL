...
namespace ObjectGL
{
    public class Texture1D : Texture
    {
            ...
            : base(TextureTarget.Texture1D, internalFormat, 1, CalculateMipCount(width))
        {
            ...
            currentContext.BindTexture(TextureTarget.Texture1D, handle);

            int widthLoc = width;
            for (int i = 0; i < mipCount; i++)
            {
                GL.TexImage1D(TextureTarget.Texture1D, i, internalFormat, widthLoc, 0, format, type, initialDataForMip(i));
                widthLoc = Math.Max(widthLoc / 2, 1);
            }
        }
            ...
            : base(TextureTarget.Texture1D, internalFormat, 1, CalculateMipCount(width))
        {
            ...
            for (int i = 0; i < mipCount; i++)
            {
                GL.CompressedTexImage1D(TextureTarget.Texture1D, i, internalFormat, widthLoc, 0, getComressedImageSizeForMip(i), getCompressedInitialDataForMip(i));
                widthLoc = Math.Max(widthLoc / 2, 1);
            }
        }
    }
